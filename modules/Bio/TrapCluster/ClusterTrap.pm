...
sub disc_ranges () {
	...
	foreach my $coords (@{$self->trapmap_arrayref}) {
		...
		if ($start > $end) {
			print "ERROR $start - $end... iinverting the coords!!!\n";
			exit;
		}
	...
	my @disc_ranges = Bio::Range->disconnected_ranges(sort {$a->start <=> $b->start} @ranges);
...
sub run{
	...
	foreach my $r (@{$disc_ranges}) {
		...
		my $maxicluster_accession = $self->get_next_accession ('maxiclustermap','MCL');
		...
		my $sql_map = "select distinct tm.trap_id, tm.trapmap_id from trap t, trapmap tm, trapblock tb where tm.start <= '".$r->end."' and tm.end >= '".$r->start."' order by tm.start,tm.end;";
		...
		$debug && print STDOUT "Disconnecting ranges\n".Dumper @ranges."\n";
		my @tb_disc_ranges = Bio::Range->disconnected_ranges(sort {$a->start <=> $b->start} @ranges);
		$debug && print STDOUT "AFTER:\n". Dumper @tb_disc_ranges;					
		...
		foreach my $r (@tb_disc_ranges) {
			...
			my $slice = $self->annotation->slice_core_adaptor->fetch_by_region ($region,$chr,$r->start,$r->end,$str);
			
			$toinsert_maxiclusterblocks{'sequence'} = $slice->seq();
			
			my $maxiclusterblocks_id = $self->load->load_maxiclusterblocks(\%toinsert_maxiclusterblocks);
		}
		...
		my $maxicluster_seq = $self->get_seq_from_id ('maxiclusterblock',  $maxiclustermap_id, $str);
		...
		$self->create_trapcluster($region,$maxicluster_id, $maxiclustermap_id, $hit_db, $chr, $str, $debug);
	}
}
...
sub create_trapcluster{                                                                                                                                                                                                         
	...
	foreach my $blocks (@{$self->load->fetch->select_many_from_table($sql)}) {
		...
		if ($start && $end) {
			...
			my $overlapping_trapblocks = 0;
			
			$self->find_overlapping_traps($region,$maxicluster_id, $maxiclustermap_id, \%blocks, $overlapping_trapblocks, $hit_db, $chr, $strand, $debug);
		}
	}
}

sub find_overlapping_trap{
	
	my ($self,$region,$maxicluster_id, $maxiclustermap_id, $blocks, $overlapping_trapblocks, $hit_db, $chr, $strand, $debug) = @_;
	my %blocks = %$blocks;
	my ($where,$where2);
	my $i = 0;
	...
	if ($j == $overlapping_trapblocks) {
		...
		if ($trapcluster_id) {
			...
			my $trapclustermap_id = $self->load->load_trapclustermap(\%trapclustermap_toinsert);	
		}
		
		my @features;
		foreach my $block_id (keys %blocks) {
			my $sql_tclblock = "select start,end,sequence from maxiclusterblock where maxiclusterblock_id = '$block_id';";
		...
		my $trapcluster_seq = $self->get_seq_from_id ('trapclusterblock',  $trapclustermap_id, $strand, $debug);
		...
		$self->annotate($region,\@features,$trapcluster_id);
		
	}
}

sub annotate{
	my ($self,$region,$feats,$trapcluster_id) = @_;
	
	my $trapcluster = $self->laod->fetch->get_trapcluster_by_id($trapcluster_id);
	
	foreach my $feat (@{$feats}){
		
		my $slice_core_adaptor = $self->annotation->slicecoreadpt;
		my $slice_core = $slice_core_adaptor->fetch_by_region($region,$feat->{'hit_id'}, $feat->{'start'}, $feat->{'end'});
		my $found = 0;
		if ($self->do_ensgene) {
		...
		if ($self->do_ensestgene){
			...
			my $slice_est = $slice_est_adaptor->fetch_by_region($region,$feat->{'hit_id'}, $feat->{'start'}, $feat->{'end'});
...
sub annotate_with_ensembl_genescan {
	...
	my $trapblock = $self->load->fetch->get_selected_trapblock_by_trapclustermap_id($trapclustermap_id);
	...
	foreach my $gene (@{$slice->get_all_PredictionTranscripts}) {
		...
		if ($ti->{'i'}) {
			...
			my $update_trapcheck = qq{UPDATE trapcheck SET annotated = 1, checked = 1, mapped = 1 WHERE trapcluster_id = $trapcluster_id};
	...
	if ($yes_gene) {
		...
		$self->load->load_trapclustermap_region($hash->{'trapclustermap_region'});
		my $update = qq{UPDATE trapcheck SET annotated = 0, checked = 1 WHERE trapcluster_id = $trapcluster_id};
