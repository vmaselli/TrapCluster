...
sub create_db () {
    ...
    $self->db_connection->do("create database $dbname");
...
sub drop_db () {
    ...
    my $res;
    if ($self->pass) {
    	`$mysql -u $user -p$pass -h $host DROP $dbname`;
    }
    else {
    	$self->db_connection->do("drop database $dbname");
		#`$mysql -u $user -h $host DROP $dbname`;
    }
    return 1;
}
...
sub exec_import () {
    ...
    my $res;
    if ($self->pass) {
	    $res = system $mysql_path."mysql -u $user -p$pass -h $host $db < $file";
	    }
	    else {
	    $res = system $mysql_path."mysql -u $user -h $host $db < $file";
	    }
    #print  $mysql_path."mysql -u $user -p$pass -h $host $db < $file\n";
    unless ($res){$self->{'exec_import'} = 1}
    else{print "cannote import $file : $DBI::errstr\n";$self->{'exec_import'} = 0}
...
sub prepare_stmt {
	...
	eval {
		$dbh->begin_work();
   		$sth = $dbh->prepare($stmt)|| die "cannote prepare $stmt : $DBI::errstr";
   		$dbh->commit();
   		$success = 1;
   	};
   	if ($@) {
	   ...
	   if ($error == ER_LOCK_WAIT_TIMEOUT ||
		   $error == ER_LOCK_DEADLOCK) {
		 # It's a temporary problem, so we can re-try the transaction
		 $retry = 1;
	
		 $dbh->rollback();
	   } else {
		 # It's a more serious problem, and we probabyl can't re-try
		 $retry = 0;
	   }
...
sub import_schema {
    ...
    die "cannote prepare $stmt : $DBI::errstr" unless $sth;
    $sth->execute;
    $self->{'import_schema'} = $res;
...
sub exec_dump () {
    ...
    eval{
    if ($self->pass) {
	    system $mysql_path."mysqldump -u $user -p$pass -h $host $attr $db > $file";
	    }
	else {
	    system $mysql_path."mysqldump -u $user -h $host $attr $db > $file";
	}
	};
