...
=head2 disconnected_ranges
 ...
 Usage   : my $ranges = Bio::MCE::Ranges->disconnected_ranges($dbh,$ranges,'1');

                                      OR

           my $ranges = Bio::MCE::Ranges->disconnected_ranges($dbh,$sth,'1','STH');
 ...
 Args    : -1 dbh
           -2 a NOT fetched sth of feature_pair or array_ref of Bio::Range
           -3 '1' if you want to drop the table created for calculation
           -4 'STH' or ('q' or 't' if from genomic_pw) if you are using a NOT fetched 'sth' object and not an
              array_ref of Bio::Range
...
sub disconnected_ranges {
	...
	if($sth) {
		if($sth eq 'q' || $sth eq 't') {
			$nrows = _populate_table_using_genomic_sth($dbh,$table,$features,$sth);
		}
		elsif($sth eq 'STH') {
			$nrows = _populate_table_using_sth($dbh,$table,$features);
		}
	}

	else {
		$nrows = _populate_table($dbh,$table,$features);
	}
...
sub _create_table {
	...
	my $dbh = shift;
	my $table = Bio::MCE::Utils->generate_random_string('10');
...
sub _populate_table_using_genomic_sth {
  ...
  while(my $feature = $features->fetchrow_hashref) {
    my $insert = $dbh->prepare("INSERT INTO $table SET ".
                               'start = '.$dbh->quote($feature->{"$type\_chr_start"}).
                               ',end = '.$dbh->quote($feature->{"$type\_chr_end"}));
    $insert->execute;
  }
  my $nrows = _get_num_rows($dbh,$table);
  return($nrows);
...
sub _populate_table_using_sth {
  ...
  while(my $feature = $features->fetchrow_hashref) {
    my $insert = $dbh->prepare("INSERT INTO $table SET ".
                               'start = '.$dbh->quote($feature->{'q_start'}).
                               ',end = '.$dbh->quote($feature->{'q_end'}));
    $insert->execute;
  }
  my $nrows = _get_num_rows($dbh,$table);
  return($nrows);
...
=head2 curtained_ranges
 ...
 Usage   : my $ranges = Bio::MCE::Ranges->curtained_ranges($ranges);
